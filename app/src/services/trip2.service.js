...
const create = async (data) => {
  ...
  try {
    ...
    const group = await Group.create({ seperatePeople: true, numberOfPeople, GroupLeader: leaderId, notes: notes, TripId: createdTrip.id }, { transaction: transaction });
...
const update = async (id, data) => {
  ...
  try {
    ...
    // Create trip
    const updatedTrip = await Trip.findByPk(data?.trip?.id, {
      transaction,
    });
    if (!updatedTrip) {
    ...
    // Create group of people for reservation
    const updatedGroup = await Group.findByPk(id, { transaction });
